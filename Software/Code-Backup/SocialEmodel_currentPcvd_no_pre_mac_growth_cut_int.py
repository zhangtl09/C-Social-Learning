        ...
        if (self.how_many_act_social>=1):
            ...
            for i in range(Num_groups):
                ...
                if (Num_agent_in_group<=1):
                    ...
                    #######################growth9
                    Pro_solution=[fi.subs(zip(Population_sym, self.PSocial)) for fi in Solution]
                    Gro_solution=[fi.subs(zip(Population_sym, self.MrkvNowPcvd)) for fi in Solution]
                    #######################growth9
                    for index_of_index in range(Num_agent_in_group):
                        Social_pos=Group[i][index_of_index]
                        #######################change10
                        self.PSocial[Social_pos]=Pro_solution[index_of_index]
                        self.MrkvNowPcvd[Social_pos]=int(round(Gro_solution[index_of_index]))
                        #######################change10
