...
class StickyEconsumerType(AggShockConsumerType):
    ...
    def getStates(self):
        ...
        for SocialCounter in range(self.AgentCount):
            ...
            self.correction=self.Pcvd_lag[SocialCounter]-self.pLvlNow[SocialCounter]
            #self.correction=-self.Pcvd_lag[SocialCounter]
            ##################            
            #Test[SocialCounter]=self.Pcvd_lag[SocialCounter]-self.Pcvd_lag[SocialCounter]
            ############3
            Left_sum=sum(self.pLvlNow[(SocialCounter-self.NetSiz):(SocialCounter)])
            Right_sum=sum(self.pLvlNow[(SocialCounter):(SocialCounter+self.NetSiz+1)])
            if ((SocialCounter-self.NetSiz)<0):
                Left_sum=sum(self.pLvlNow[0:(SocialCounter)])+sum(self.pLvlNow[(self.AgentCount+SocialCounter-self.NetSiz):])
            if ((SocialCounter+self.NetSiz+1)>self.AgentCount):
                Right_sum=sum(self.pLvlNow[SocialCounter:])+sum(self.pLvlNow[0:(SocialCounter+self.NetSiz+1-self.AgentCount)])
