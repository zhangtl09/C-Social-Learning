    ...
    if (Num_agent_in_group<=1):
        ...
        if (Social_pos+NetSiz>(AgentCount-1)):
            if ((Social_pos+1)>(AgentCount-1)):
                ...
                #########################growth5                
        PSocial[Social_pos]=(Left_sum_char+Right_sum_char)/float(2*NetSiz)
        ########################growth6
        ...
        ##########################growth9
        #Pro_solution=[fi.subs(zip(Population_sym, PSocial)) for fi in Solution]
        #Gro_solution=[fi.subs(zip(Population_sym, Growth_social)) for fi in Solution]
        ##########################growth9
        for index_of_index in range(Num_agent_in_group):
            Social_pos=Group[i][index_of_index]
            ##########################change10    
            Name_set=list(Solution[index_of_index].free_symbols)
            Pro_Value_set=[PSocial[int(str(Name)[1:])] for Name in Name_set]
            Gro_Value_set=[Growth_social[int(str(Name)[1:])] for Name in Name_set]
            PSocial[Social_pos]=Solution[index_of_index].subs(zip(Name_set,Pro_Value_set))
            Growth_social[Social_pos]=int(round(Solution[index_of_index].subs(zip(Name_set,Gro_Value_set))))
            ##########################change10    
