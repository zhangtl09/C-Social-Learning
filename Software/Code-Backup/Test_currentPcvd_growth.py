...
#        Value_set=[Growth_social[int(Name[1:])] for Name in Value_set_name]
#        Prob_set=[Poly(expre).coeffs() for expre in Solution]
#        #######################growth9
        for index_of_index in range(Num_agent_in_group):
            Social_pos=Group[i][index_of_index]
            #######################growth10
            ...
            Pro_Value_set=[Pcvd[int(str(Name)[1:])] for Name in Name_set]
            PSocial[Social_pos]=Solution[Social_sym[index_of_index]].subs(zip(Name_set,Pro_Value_set))
            #######################growth10
#            #######################change10
#            #PSocial[Social_pos]=Pro_solution[index_of_index]
#            #######################change10
#            #######################growth11
#            Growth_social[Social_pos]=np.random.choice(Value_set,1,p=Prob_set[index_of_index])
#            #######################growth11
t1=time.clock()
