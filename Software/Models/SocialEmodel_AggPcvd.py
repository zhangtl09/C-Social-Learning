...
class StickyEconsumerType(AggShockConsumerType):
    ...
    def simBirth(self,which_agents):
        ...
        '''
        #################Debug##########
        self.pLvl1=self.pLvlNow
        AggShockConsumerType.simBirth(self,which_agents)
        self.pLvl2=self.pLvlNow
        self.DiffpLvl=self.pLvl1-self.pLvl2
        self.Plvelbet=self.PlvlAggNow
        ################Debug###########
        if hasattr(self,'pLvlErrNow'):
            self.pLvlErrNow[which_agents] = 1.0
            #self.pSocial[which_agents] = (self.pSocial[which_agents]*float(2*self.NetSiz+1)-self.Pcvd_lag[which_agents]+self.PlvlAggNow)/(2*self.NetSiz+1)
            self.pSocial[which_agents] = self.PlvlAggNow           
            self.Pcvd_lag[which_agents] = self.PlvlAggNow
            ...
            self.Pcvd_lag = np.ones(self.AgentCount)
    ...
    def getStates(self):
        ...
        self.Pcvd_lag=self.pLvlNow/self.pInd
        #self.Pcvd_lag=self.PlvlAggNow/self.pLvlErrNow
        ###############################################
        ...
        for SocialCounter in range(self.AgentCount):
            ...
            self.correction=self.Pcvd_lag[SocialCounter]-self.Pcvd_lag[SocialCounter]
            #self.correction=-self.Pcvd_lag[SocialCounter]
            ##################            
            #Test[SocialCounter]=self.Pcvd_lag[SocialCounter]-self.Pcvd_lag[SocialCounter]
            ############3
            Left_sum=sum(self.Pcvd_lag[(SocialCounter-self.NetSiz):(SocialCounter)])
            Right_sum=sum(self.Pcvd_lag[(SocialCounter):(SocialCounter+self.NetSiz+1)])
            if ((SocialCounter-self.NetSiz)<0):
                Left_sum=sum(self.Pcvd_lag[0:(SocialCounter)])+sum(self.Pcvd_lag[(self.AgentCount+SocialCounter-self.NetSiz):])
            if ((SocialCounter+self.NetSiz+1)>self.AgentCount):
                Right_sum=sum(self.Pcvd_lag[SocialCounter:])+sum(self.Pcvd_lag[0:(SocialCounter+self.NetSiz+1-self.AgentCount)])
...
class StickyEmarkovConsumerType(AggShockMarkovConsumerType,StickyEconsumerType):
    ...
    def getpLvlError(self):
        ...
        #pLvlErr[self.social_act] = (self.Pcvd_lag[self.social_act]*self.PermShkAggNow)/(self.PermGroFacAgg[self.MrkvNowPcvd[self.social_act]]*self.pSocial[[self.social_act]])        
        return pLvlErr
