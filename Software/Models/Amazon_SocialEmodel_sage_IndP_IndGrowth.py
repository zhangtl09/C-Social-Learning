        ...
        if (self.how_many_act_social>=1):
            ...
            for i in range(Num_groups):
                ...
                if (Num_agent_in_group<=1):
                    ...
                    ############Sage1
                    a='a0'
                    for j in range(1, Num_agent_in_group+1):
                        a = a + ',a' + str(j)
                    Social_sym=var(a)
                    ...
                    ##################sage2
                    M = 'M0'
                    for j in range(1, self.AgentCount+1):
                        M = M + ',M' + str(j)
                    Population_sym=var(M)
                    ...
                    #####################change9
                    ###############sage4
                    Solution=solve(Equations,Social_sym)
                    Solution_p=solve(Equations_p,Social_sym_p)
                    ###############sage4
#                    self.solution_test=Solution
#                    self.Eq=Equations
#                    self.Social_symbol=Social_sym
#                    self.Social_sym_p=Social_sym_p
#                    self.solp=Solution_p
#                    self.Eq_p=Equations_p
#                    self.Left_p=Left_sum_p
#                    self.Right_p=Right_sum_p
#                    self.V_dummy_p=Variable_dummy_p
#                    self.time_end=clock()
                    #####################change9 unfinished*********
                    for index_of_index in range(Num_agent_in_group):
                        self.index=index_of_index
                        Social_pos=Group[i][index_of_index]
                        #######################growth10
                        Name_set=natsorted([str(Name) for Name in list(Solution[0][index_of_index].variables())[0:-1]])
                        Gro_Value_set=[self.MrkvNowPcvd[int(Name[1:])] for Name in Name_set]
                        Prob_set_I=[Vterm.coefficients()[0][0] for Vterm in Solution[0][index_of_index].operands()[1].operands()]
                        #Prob_set_I=Poly(Solution[Social_sym[index_of_index]]).coeffs()
                        Prob_set=map(abs,Prob_set_I)
                        Total_prob=sum(Prob_set)
                        Prob_set=list(np.array(Prob_set)/Total_prob)
                        self.MrkvNowPcvd[Social_pos]=np.random.choice(Gro_Value_set,1,p=Prob_set)
                        self.PSocial[Social_pos]=Solution_p[0][index_of_index].operands()[1]
                        #######################growth10                        
