...
class StickyEconsumerType(AggShockConsumerType):
    ...
    def getShocks(self):
        ...
        #########################################
        #Calculate perceived productivity for socially update agents when the probability of social updating is greater than zero
        if (self.how_many_act_social>=1):
            self.PSocial=self.Pcvd
            ...
            for i in range(Num_groups):
                ...
                if (Num_agent_in_group<=1):
                    ...
                    self.PSocial[Social_pos]=(Left_sum_char+Right_sum_char+self.Pcvd_pre[Social_pos])/float(2*self.NetSiz+1)
                    ...
                    Variable_dummy=Variable_dummy*self.pInd
                    for index_of_index in range(Num_agent_in_group):
                        ...
                        s=(Left_sum+Right_sum+self.Pcvd_pre[Social_pos])/float(2*self.NetSiz+1)-Social_sym[index_of_index]
                        Equations.append(s)
