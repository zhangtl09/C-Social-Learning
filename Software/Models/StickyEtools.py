...
def makeStickyEdataFile(Economy,ignore_periods,description='',filename=None,save_data=False,calc_micro_stats=True,meas_err_base=None):
    ...
    if hasattr(Economy,'agents'): # If this is a heterogeneous agent specification...
        ...
        if calc_micro_stats: # Only calculate stats if requested.  This is a memory hog with many simulated periods
            ...
            print('micro_stat_periods')
            print(micro_stat_periods)
            #print(sum(np.isnan(micro_stat_periods)))
            print('**************************')
            ...
            print('Not Newborn')            
            Not_new_born_path='../../../Result/'+'NotNewBorn.txt'
            print(sum(np.isnan(not_newborns)))
            np.savetxt(Not_new_born_path, not_newborns, delimiter="\n")
            print('**************************')
            Logc = np.log(cLvlAll_hist[ignore_periods:(ignore_periods+micro_stat_periods),:])
            print('Log c')            
            Logc_Path='../../../Result/'+'Logc.txt'
            print(sum(np.isnan(Logc)))
            np.savetxt(Logc_Path, Logc, delimiter="\n")
            print('**************************')
            DeltaLogc = (Logc[1:] - Logc[0:-1]).flatten()
            print('DeltaLog c')            
            Delta_Logc_Path='../../../Result/'+'DeltaLogc.txt'
            print(sum(np.isnan(DeltaLogc)))
            np.savetxt(Delta_Logc_Path, DeltaLogc, delimiter="\n")
            print('**************************')
            DeltaLogc_trimmed = DeltaLogc[not_newborns]
            print('Trimmed c')            
            Delta_Logc_Path_trimmed='../../../Result/'+'DeltaLogc_trimmed.txt'
            print(sum(np.isnan(DeltaLogc_trimmed)))
            print(str(len(DeltaLogc))+'length_of_not_newborn: '+str(len(not_newborns)))
            np.savetxt(Delta_Logc_Path_trimmed, DeltaLogc_trimmed, delimiter="\n")
            print('**************************')
    ...
    if hasattr(Economy,'agents') and calc_micro_stats: # This block only runs for heterogeneous agents specifications
        csv_output_string += ","+str(np.mean(np.std(Loga,axis=1)))+ ","+str(np.mean(np.std(Logc,axis=1))) + ","+str(np.mean(np.std(Logp,axis=1))) +","+ str(np.mean(np.nanstd(Logy_trimmed,axis=1))) +","+ str(np.std(DeltaLoga_trimmed))+","+ str(np.nanstd(DeltaLogc_trimmed))+ ","+str(np.std(DeltaLogp_trimmed))
...
def runStickyEregressions(infile_name,interval_size,meas_err,sticky,all_specs):
    ...
    t_stat_array = old_div(CoeffsArray,StdErrArray)
...
def extractSampleMicroData(Economy, num_periods, AgentCount, ignore_periods):
    ...
    trans_shk_matrix = old_div(total_trans_shk_matrix,(np.array(agg_trans_shk_matrix)*np.array(wRte_matrix))[:,None])
    ...
    a_matrix_nrm = old_div(a_matrix,pLvlTrue_matrix)
...
def makeuCostVsPiFig(uCost_filename):
    ...
    plt.plot([old_div(1.,UpdatePrbBase),old_div(1.,UpdatePrbBase)],[0.,f(UpdatePrbBase)],'--k') # Add dashed line at Pi=0.25
    plt.xlim([1.0,16.0])
    plt.ylim([0.0,35.0])
